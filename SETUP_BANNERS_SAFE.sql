-- Safe version that doesn't break existing triggers
-- Only creates banner_content table and policies without touching shared functions

-- Drop existing banner policies only (safe)
DROP POLICY IF EXISTS "Authenticated users can view banner content" ON banner_content;
DROP POLICY IF EXISTS "Authenticated users can manage banner content" ON banner_content;

-- Drop existing banner trigger only (safe)
DROP TRIGGER IF EXISTS update_banner_content_updated_at ON banner_content;

-- Create banner_content table (will skip if exists)
...
-- Add RLS (Row Level Security) - safe to run multiple times
ALTER TABLE banner_content ENABLE ROW LEVEL SECURITY;

-- Create policies for authenticated users to read banner content
...
-- Add index for faster lookups - safe to run multiple times
CREATE INDEX IF NOT EXISTS idx_banner_content_page_key ON banner_content(page_key);

-- Create banner-specific trigger using existing function
CREATE TRIGGER update_banner_content_updated_at 
  BEFORE UPDATE ON banner_content 
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Success message
SELECT 'Banner content table setup completed successfully!' as result;
