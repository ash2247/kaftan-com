-- Deploy Safari and Paradise Products with Exact Status Detection
-- This script will find the exact allowed status values and use them

-- Step 1: Find the exact constraint definition
DO $$
DECLARE
    constraint_def TEXT;
    allowed_status TEXT;
BEGIN
    ...
    FROM pg_constraint 
    WHERE conrelid = 'products'::regclass AND contype = 'c' AND conname = 'products_status_check';
    
    -- Try to extract allowed status values from the constraint definition
    -- Common patterns to check for
    IF constraint_def LIKE '%draft%' THEN
        allowed_status := 'draft';
    ELSIF constraint_def LIKE '%published%' THEN
        allowed_status := 'published';
    ELSIF constraint_def LIKE '%active%' THEN
        allowed_status := 'active';
    ELSIF constraint_def LIKE '%inactive%' THEN
        allowed_status := 'inactive';
    ELSIF constraint_def LIKE '%pending%' THEN
        allowed_status := 'pending';
    ELSIF constraint_def LIKE '%available%' THEN
        allowed_status := 'available';
    ELSIF constraint_def LIKE '%enabled%' THEN
        allowed_status := 'enabled';
    ELSIF constraint_def LIKE '%disabled%' THEN
        allowed_status := 'disabled';
    ELSIF constraint_def LIKE '%true%' THEN
        allowed_status := 'true';
    ELSIF constraint_def LIKE '%false%' THEN
        allowed_status := 'false';
    ELSIF constraint_def LIKE '%1%' THEN
        allowed_status := '1';
    ELSIF constraint_def LIKE '%0%' THEN
        allowed_status := '0';
    ELSE
        -- If we can't determine from constraint, try existing products
        SELECT status INTO allowed_status 
        FROM products 
        LIMIT 1;
    END IF;
    
    -- Store the allowed status for later use
    CREATE TEMP TABLE IF NOT EXISTS temp_status AS SELECT allowed_status as status_value;
END $$;

-- Step 2: Check if display_page column exists, add if it doesn't
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Step 3: Check if slug column exists, add if it doesn't
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- Step 4: Update existing products to have slugs if they don't exist
UPDATE products 
SET slug = LOWER(REGEXP_REPLACE(REGEXP_REPLACE(name, '[^a-zA-Z0-9\s-]', '', 'g'), '\s+', '-'))
WHERE slug IS NULL OR slug = '';

-- Step 5: Insert Safari Products using the detected status
...
SELECT 
    ...
    (SELECT status_value FROM temp_status LIMIT 1) as status,
    true as in_stock, 
    images, 
    NOW()
FROM (
...
-- Step 6: Insert Paradise Products using the same detected status
INSERT INTO products (
    id, name, slug, price, category, style, color, display_page, status, in_stock, images, created_at
) 
SELECT 
...
-- Step 9: Show the constraint definition for reference
SELECT 'CONSTRAINT DEFINITION' as info, pg_get_constraintdef(oid) as definition
FROM pg_constraint 
WHERE conrelid = 'products'::regclass AND contype = 'c' AND conname = 'products_status_check';
