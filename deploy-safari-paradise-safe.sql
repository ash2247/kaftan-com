-- Deploy Safari and Paradise Products to Live Database (SAFE VERSION)
-- Run this script in your LIVE Supabase SQL Editor

-- Step 1: Find the correct status value to use
-- First, let's see what status values are already used successfully
DO $$
DECLARE
    correct_status TEXT;
BEGIN
    -- Try to find a working status value by checking existing products
    SELECT status INTO correct_status 
    FROM products 
    WHERE status IN ('active', 'published', 'draft', 'available', 'true', '1', 'enabled')
    LIMIT 1;
    
    -- If no existing products found, default to 'active' and handle the error
    IF correct_status IS NULL THEN
        correct_status := 'active';
    END IF;
    
    -- Store the correct status in a temporary table for later use
    CREATE TEMP TABLE IF NOT EXISTS temp_status AS SELECT correct_status as status_value;
END $$;
...
-- Step 5: Insert Safari Products (using the correct status)
...
-- Step 6: Insert Paradise Products (using the correct status)
INSERT INTO products (
    id, name, slug, price, category, style, color, display_page, status, in_stock, images, created_at
) 
SELECT 
...
-- Step 8: Verification - Check results
